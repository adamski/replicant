...
async fn test_simple_event_delivery() {
    ...
    client2.event_dispatcher().register_rust_callback(
        Box::new(move |event_type, document_id, title, _content, _error, _numeric_data, _boolean_data, _context| {
            ...
            match event_type {
                EventType::DocumentCreated => {
                    if let (Some(doc_id), Some(title_str)) = (document_id, title) {
                        ...
                        tracing::info!("Client 2 received DocumentCreated event for {}", title_str);
                    }
                }
                _ => {}
            }
    ...
    let events = events_received.lock().unwrap();
    panic!("Client 2 never received the create event. Events: {:?}", *events);
}