use crate::integration::helpers::{assert_all_clients_converge, TestContext};
...
crate::integration_test!(
    test_multiple_clients_same_user_create_update_delete,
    |ctx: TestContext| async move {
        ...
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-multi-client")
            .await
        ...
        assert_all_clients_converge(&[&client1, &client2, &client3], 1, 5, |doc| {
...
crate::integration_test!(
    test_concurrent_document_creation_same_user,
    |ctx: TestContext| async move {
        if std::env::var("RUN_INTEGRATION_TESTS").is_err() {
            eprintln!("Skipping integration test. Set RUN_INTEGRATION_TESTS=1 to run.");
            return;
        ...
        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-concurrent")
            .await
            .expect("Failed to generate credentials");
        ...
        tracing::info!(
            "Creating documents concurrently from {} clients",
            num_clients
        );
        ...
        for (i, result) in results.iter().enumerate() {
            assert!(
                result.is_ok(),
                "Client {} failed to create document: {:?}",
                i,
                result
            );
        }
        ...
        tracing::info!(
            "✓ All {} documents created concurrently are visible to all clients",
            num_clients
        );
    },
    true
...
crate::integration_test!(
    test_no_duplicate_broadcast_to_sender,
    |ctx: TestContext| async move {
        if std::env::var("RUN_INTEGRATION_TESTS").is_err() {
            eprintln!("Skipping integration test. Set RUN_INTEGRATION_TESTS=1 to run.");
            return;
        ...
        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-no-dup")
            .await
            .expect("Failed to generate credentials");
        ...
        assert_eq!(
            docs.len(),
            1,
            "Expected exactly 1 document, found {}",
            docs.len()
        );
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-offline")
            .await
            .expect("Failed to generate credentials");
        ...
        let doc1 = client1
            .create_document(json!({ "title": "Document 1", "created": "while client 2 offline" }))
            .await
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        // Create a document
        let doc = client1
            .create_document(json!({ "title": "Counter Document", "counter": 0 }))
            .await
            .expect("Failed to create document");
        ...
        // This avoids the need to clone Client
        ...
        assert_eq!(
            final_doc1.sync_revision, final_doc2.sync_revision,
        ...
        tracing::info!("  Final sync_revision: {}", final_doc1.sync_revision);
