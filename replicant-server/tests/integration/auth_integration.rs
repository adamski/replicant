...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let _doc = client
            .create_document(json!({"title": "Demo Test Doc", "test": true}))
            .await
            .expect("Failed to create document");

        // Verify sync worked
        let synced_docs = client
            .get_all_documents()
            .await
...
crate::integration_test!(
    test_custom_token_auto_registration,
    |ctx: TestContext| async move {
        ...
        // Create a document
        let _doc = client1
            .create_document(json!({"title": "Auto Registration Test", "test": true}))
            .await
            .expect("Failed to create document");
        ...
        assert_eq!(
            docs.len(),
            1,
            "Client2 should see 1 document but found {}",
            docs.len()
        );
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        // Create a document
        let _doc = client
            .create_document(json!({"title": "Security Test", "test": true}))
            .await
            .expect("Failed to create document");
        ...
        let result = replicant_client::Client::new(
            &db_path,
            &ws_url,
            email,
            "rpa_wrong_key",
        ...
        assert!(
            result.is_err(),
            "Creating engine with invalid credentials should fail"
        );
    },
    true
);
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let _doc0 = client0
            .create_document(json!({"title": "Doc from client 0", "test": true}))
            .await
            .expect("Failed to create document 0");
        ...
        for i in 1..5 {
            ...
            let _doc = client
                .create_document(json!({"title": format!("Doc from client {}", i), "test": true}))
                .await
        ...
        loop {
            ...
            if start.elapsed() > timeout {
                ...
                eprintln!(
                    "Convergence timeout! Client states after {} seconds:",
                    timeout.as_secs()
                );
                ...
                panic!(
                    "Clients did not converge within {} seconds",
                    timeout.as_secs()
                );
            }
        ...
        for (i, client) in clients.iter().enumerate() {
            ...
            assert_eq!(
                docs.len(),
                5,
                "After convergence, client {} has {} documents",
                i,
                docs.len()
            );
        }
