...
#[tokio::test]
async fn test_offline_conflict_detection_and_resolution() {
    ...
    let (api_key, api_secret) = ctx.generate_test_credentials(&format!("conflict-test-{}", test_id))
        .await
        .expect("Failed to generate credentials");
    let user_id = ctx.create_test_user(&email)
        .await
    ...
    // Create two clients
    let client_a = ctx.create_test_client(&email, user_id, &api_key, &api_secret)
        .await
        .expect("Failed to create client A");

    let client_b = ctx.create_test_client(&email, user_id, &api_key, &api_secret)
        .await
    ...
    let doc = client_a.create_document(original_content.clone())
        .await
    ...
    while start.elapsed() < max_wait {
        let docs_b = client_b.get_all_documents().await.expect("Failed to get docs");
    ...
    assert!(synced_to_b, "Document should sync to client B within 5 seconds");

    // Both clients now have the same document
    let docs_a = client_a.get_all_documents().await.expect("Failed to get docs");
    let docs_b = client_b.get_all_documents().await.expect("Failed to get docs");
    ...
    client_a.update_document(doc_id, edit_a.clone())
        .await
    ...
    client_b.update_document(doc_id, edit_b.clone())
        .await
    ...
    let final_docs_a = client_a.get_all_documents().await.expect("Failed to get docs");
    let final_docs_b = client_b.get_all_documents().await.expect("Failed to get docs");
    ...
    tracing::info!("Client A revision: {}", final_a.revision_id);
    tracing::info!("Client B revision: {}", final_b.revision_id);
    ...
    assert_eq!(
        final_a.revision_id, final_b.revision_id,
        "Clients should have the same revision after conflict resolution"
    );

    tracing::info!("✅ Conflict resolution test passed - clients converged to consistent state");
}
...
#[tokio::test]
async fn test_conflict_events_logged_correctly() {
    let ctx = TestContext::new();

    let test_id = uuid::Uuid::new_v4();
    let email = format!("conflict-events-test-{}@example.com", test_id);
    let (api_key, api_secret) = ctx.generate_test_credentials(&format!("conflict-events-test-{}", test_id))
        .await
        .expect("Failed to generate credentials");
    let user_id = ctx.create_test_user(&email)
        .await
    ...
    // Create a document
    let doc = client.create_document(json!({"value": 1}))
        .await
        .expect("Failed to create document");
    ...
    for i in 2..5 {
        client.update_document(doc.id, json!({"value": i}))
            .await
    ...
    let docs = client.get_all_documents().await.expect("Failed to get docs");
