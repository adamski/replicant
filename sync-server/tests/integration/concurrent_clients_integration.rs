    ...
    |ctx: TestContext| async move {
        ...
        for i in 0..client_count {
            ...
            let handle = tokio::spawn(async move {
                let client = ctx_clone
                    .create_test_client(email, user_id, &api_key_clone, &api_secret_clone)
                    .await
    ...
    |ctx: TestContext| async move {
        ...
        for user_idx in 0..user_count {
            ...
            let handle = tokio::spawn(async move {
                ...
                let (api_key, api_secret) = ctx_clone
                    .generate_test_credentials(&format!("test-user{}", user_idx))
                    .await
                    .expect("Failed to generate credentials");

                // Create user
                let user_id = ctx_clone
                    .create_test_user(&email)
                    .await
                    .expect("Failed to create user");
