...
crate::integration_test!(
    test_concurrent_edit_conflict_resolution,
    |ctx: TestContext| async move {
        ...
        let doc = client1
            .create_document(json!({"title": "Conflict Test", "test": true}))
            .await
        ...
        client1
            .update_document(doc.id, json!({"text": "Client 1 edit", "version": 1}))
            .await
            .expect("Failed to update document");

        client2
            .update_document(doc.id, json!({"text": "Client 2 edit", "version": 2}))
            .await
            .expect("Failed to update document");
        ...
        // Content should be the same (last-write-wins based on sync_revision)
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let doc = client1
            .create_document(json!({"title": "Delete-Update Conflict", "test": true}))
            .await
            .expect("Failed to create document");
        // Wait for automatic sync
        tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
        ...
        client2
            .update_document(doc.id, json!({"text": "Updated while being deleted"}))
            .await
            .expect("Failed to update document");
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        for _ in 0..5 {
            clients.push(
                ctx.create_test_client(email, user_id, &api_key, &api_secret)
                    .await
                    .expect("Failed to create client"),
            );
        }
        ...
        let doc = clients[0]
            .create_document(json!({"title": "Rapid Update Test", "test": true}))
            .await
            .unwrap();
        let doc_id = doc.id;
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        for i in 0..3 {
            // Client 1 update
            client1
                .update_document(doc_id, json!({"client1_update": i}))
                .await
                .unwrap();

            // Client 2 update
            client2
                .update_document(doc_id, json!({"client2_update": i}))
                .await
                .unwrap();

            // Client 3 update
            client3
                .update_document(doc_id, json!({"client3_update": i}))
                .await
                .unwrap();
        }
