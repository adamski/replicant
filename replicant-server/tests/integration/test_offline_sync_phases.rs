...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let (api_key, _) = ctx
            .generate_test_credentials("test-offline-phases")
            .await
        ...
        let docs1 = client1
            .get_all_documents()
            .await
            .expect("Failed to get docs from client1");
        let docs2 = client2
            .get_all_documents()
            .await
            .expect("Failed to get docs from client2");

        assert_eq!(docs1.len(), 3, "Client 1 should see 3 documents");
        assert_eq!(docs2.len(), 3, "Client 2 should see 3 documents");

        tracing::info!(
            "✓ Initial sync successful - both clients see {} documents",
            docs1.len()
        );
...
crate::integration_test!(
    ...
    |_ctx: TestContext| async move {
        ...
        let client1_db_path = state
            .client1_db_path
            .as_ref()
            .expect("Client 1 DB path not found");

        // Open the database directly for offline operations
        let client_db = replicant_client::ClientDatabase::new(client1_db_path)
            .await
        ...
        let offline_doc = replicant_core::models::Document {
            id: Uuid::new_v4(),
        ...
        let local_docs = client_db
            .get_all_documents()
            .await
            .expect("Failed to get local docs");
        tracing::info!(
            "Local documents after offline changes: {}",
            local_docs.len()
        );
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-offline-phases-recovery")
            .await
            .expect("Failed to generate credentials");

        let client1 = ctx
            .create_test_client(email, state.user_id, &api_key, &api_secret)
            .await
            .expect("Failed to create client 1");
        ...
        assert_eq!(
            docs1.len(),
            docs2.len(),
            "Both clients should see same number of documents"
        );
        ...
        if let Some(offline_doc_id) = state.offline_doc_id {
            ...
            assert!(
                doc1_found,
                "Client 1 should see the offline-created document"
            );
            assert!(
                doc2_found,
                "Client 2 should see the offline-created document"
            );
        ...
        if let Some(doc1_id) = state.doc1_id {
            ...
            if let (Some(d1), Some(d2)) = (doc1_client1, doc1_client2) {
                assert_eq!(
                    d1.content["offline"], true,
                    "Client 1 should see offline update"
                );
                assert_eq!(
                    d2.content["offline"], true,
                    "Client 2 should see offline update"
                );
...
crate::integration_test!(
    ...
    |_ctx: TestContext| async move {
        ...
        let email = "offline-sync-test@example.com";
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-offline-phases-verify")
            .await
            .expect("Failed to generate credentials");
        ...
        assert_eq!(
            docs1.len(),
            docs2.len(),
            "Client 1 and 2 should have same doc count"
        );
        assert_eq!(
            docs2.len(),
            docs3.len(),
            "Client 2 and 3 should have same doc count"
        );
        ...
        summary_parts.push(format!(
            "Final state: {} documents across all clients",
            docs1.len()
        ));
        ...
        if let Some(doc1_id) = state.doc1_id {
            if docs1
                .iter()
                .any(|d| d.id == doc1_id && d.content["offline"] == true)
            {
