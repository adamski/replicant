...
async fn create_client_with_event_tracking(
    ...
    api_key: &str,
    api_secret: &str,
) -> Result<(Client, Arc<Mutex<EventLog>>), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let db = replicant_client::ClientDatabase::new(&db_path).await?;
    ...
    let engine = Client::new(
        &db_path,
        &format!("{}/ws", ctx.server_url),
        email,
        api_key,
        api_secret,
    )
...
crate::integration_test!(
    test_offline_changes_sync_on_reconnect,
    |mut ctx: TestContext| async move {
        ...
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-offline-sync")
            .await
        ...
        let (client1, events1) =
            create_client_with_event_tracking(&ctx, user_id, email, &api_key, &api_secret)
                .await
        ...
        let docs1 = client1
            .get_all_documents()
            .await
            .expect("Failed to get docs from client 1");
        let docs2 = client2
            .get_all_documents()
            .await
            .expect("Failed to get docs from client 2");
        ...
        let updated_doc1 = docs1
            .iter()
            .find(|d| d.id == doc.id)
            .expect("Should find updated doc");
        let updated_doc2 = docs2
            .iter()
            .find(|d| d.id == doc.id)
            .expect("Should find updated doc");
        ...
        assert!(
            docs1.iter().any(|d| d.id == offline_doc.id),
            "Client 1's offline doc should be on client 1"
        );
        assert!(
            docs2.iter().any(|d| d.id == offline_doc.id),
            "Client 1's offline doc should be on client 2"
        );
        assert!(
            docs1.iter().any(|d| d.id == offline_doc2.id),
            "Client 2's offline doc should be on client 1"
        );
        assert!(
            docs2.iter().any(|d| d.id == offline_doc2.id),
            "Client 2's offline doc should be on client 2"
        );
        ...
        // Verify event counts after reconnection
        {
            ...
            assert!(
                e1.created.iter().any(|(id, _)| *id == offline_doc2.id),
            ...
            assert!(
                e2.updated.iter().any(|(id, _)| *id == doc.id),
                "Client 2 should have received update event"
            );
            assert!(
                e2.created.iter().any(|(id, _)| *id == offline_doc.id),
...
crate::integration_test!(
    test_task_list_scenario_with_events,
    |ctx: TestContext| async move {
        ...
        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-alice-tasks")
            .await
            .expect("Failed to generate credentials");
        ...
        sleep(Duration::from_millis(500)).await;
        {
            ...
            assert!(
                e1.updated.iter().any(|(id, _)| *id == task1.id),
                "Client 1 should see update"
            );
            assert!(
                e2.updated.iter().any(|(id, _)| *id == task1.id),
                "Client 2 should see update"
            );
            assert!(
                e3.updated.iter().any(|(id, _)| *id == task1.id),
                "Client 3 should see update"
            );
        }
        ...
        sleep(Duration::from_millis(500)).await;
        {
            ...
            assert!(
                e1.deleted.contains(&task2.id),
                "Client 1 should see deletion"
            );
            assert!(
                e2.deleted.contains(&task2.id),
                "Client 2 should see deletion"
            );
            assert!(
                e3.deleted.contains(&task2.id),
                "Client 3 should see deletion"
            );
        }
        ...
        tracing::info!(
            "✓ Task list scenario completed successfully with all events properly delivered"
        );
    },
    true
);
...
crate::integration_test!(
    test_rapid_updates_with_event_callbacks,
    |ctx: TestContext| async move {
        if std::env::var("RUN_INTEGRATION_TESTS").is_err() {
            eprintln!("Skipping integration test. Set RUN_INTEGRATION_TESTS=1 to run.");
            return;
        ...
        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-rapid")
            .await
            .expect("Failed to generate credentials");
        ...
        let doc = client1
            .create_document(json!({ "title": "Counter", "value": 0 }))
            .await
        ...
        for i in 0..update_count {
            // Client 1 update
            client1
                .update_document(doc.id, json!({ "value": i * 2, "last_client": 1 }))
                .await
            ...
            // Client 2 update
            client2
                .update_document(doc.id, json!({ "value": i * 2 + 1, "last_client": 2 }))
                .await
                .expect("Update failed");
        ...
        assert_eq!(
            docs1[0].content, docs2[0].content,
            "Clients should converge to same state"
        );
        ...
        // Verify event counts
        {
            ...
            assert!(
                e1.updated.len() >= update_count,
                "Client 1 should see at least {} updates",
                update_count
            );
            assert!(
                e2.updated.len() >= update_count,
                "Client 2 should see at least {} updates",
                update_count
            );

            // Check for conflicts (there might be some due to rapid updates)
            tracing::info!(
                "Client 1 conflicts: {}, Client 2 conflicts: {}",
                e1.conflicts.len(),
                e2.conflicts.len()
            );
        }
