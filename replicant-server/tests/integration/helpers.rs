...
use hmac::{Hmac, Mac};
use libc::kill;
use replicant_client::Client as SyncClient;
...
const SERVER_BIN: &str = env!("CARGO_BIN_EXE_replicant-server");
...
// Remove TestClient wrapper - we'll use Client directly
...
impl TestContext {
    ...
    async fn create_test_client_attempt(
        ...
        let (db_path, ws_url, _db) = {
            ...
            // Note: API credentials are NOT stored in database - they're passed to Client
            sqlx::query(
            ...
            // we need to keep our in memory db alive to create our Client
    ...
    pub async fn create_authenticated_websocket(
        ...
        if let Some(Ok(Message::Text(response))) = ws.next().await {
            use replicant_core::protocol::ServerMessage;
    ...
    pub async fn recreate_database(&self) -> Result<()> {
        ...
        let db = replicant_server::database::ServerDatabase::new(
            &self.db_url,
...
pub async fn assert_all_clients_converge<F, Fut>(
    clients: &[&replicant_client::Client],
    expected_count: usize,
    timeout_secs: u64,
    check_fn: F,
) where
    F: Fn(&replicant_core::models::Document) -> Fut + Clone,
    Fut: std::future::Future<Output = bool>,
{
