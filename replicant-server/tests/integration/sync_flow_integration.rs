...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let _doc = client1
            .create_document(json!({"title": "Sync Test Doc", "test": true}))
            .await
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let _doc1 = client1
            .create_document(json!({"title": "Doc from Client 1", "test": true}))
            .await
            .expect("Failed to create document");
        let _doc2 = client2
            .create_document(json!({"title": "Doc from Client 2", "test": true}))
            .await
            .expect("Failed to create document");
        ...
        let titles1: Vec<String> = docs1
            .iter()
            .map(|d| d.title_or_default().to_string())
            .collect();
        let titles2: Vec<String> = docs2
            .iter()
            .map(|d| d.title_or_default().to_string())
            .collect();
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        // Client 1 creates a document
        let doc = client1
            .create_document(json!({"title": "Update Test", "text": "Original content"}))
            .await
            .expect("Failed to create document");
        ...
        client1
            .update_document(doc.id, json!({"text": "Updated content"}))
            .await
...
crate::integration_test!(
    ...
    |ctx: TestContext| async move {
        ...
        let _doc1 = client1
            .create_document(json!({"title": "Keep Me", "test": true}))
            .await
            .expect("Failed to create document");
        let doc2 = client1
            .create_document(json!({"title": "Delete Me", "test": true}))
            .await
            .expect("Failed to create document");
...
crate::integration_test!(
    test_large_document_sync,
    |ctx: TestContext| async move {
        let email = "eve@test.local";

        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-eve")
            .await
            .expect("Failed to generate credentials");
...
crate::integration_test!(
    test_simultaneous_offline_outage_scenario,
    |ctx: TestContext| async move {
        ...
        let shared_doc = client1
            .create_document(
                json!({"title": "Shared Document", "content": "initial content", "version": 0}),
            )
            .await
            .expect("Failed to create shared document");
        ...
        assert_eq!(
            docs1.len(),
            1,
            "Client1 should have 1 document after initial sync"
        );
        assert_eq!(
            docs2.len(),
            1,
            "Client2 should have 1 document after initial sync"
        );
        assert_eq!(
            docs3.len(),
            1,
            "Client3 should have 1 document after initial sync"
        );
        ...
        let client1_back = ctx
            .create_test_client(email, user_id, &api_key, &api_secret)
            .await
            .expect("Failed to reconnect client1");
        ...
        println!(
            "📊 Client1 after reconnect: {} docs",
            docs_before_update.len()
        );
        ...
        let client2_back = ctx
            .create_test_client(email, user_id, &api_key, &api_secret)
            .await
            .expect("Failed to reconnect client2");
        ...
        let client3_back = ctx
            .create_test_client(email, user_id, &api_key, &api_secret)
            .await
            .expect("Failed to reconnect client3");
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await; // Let it sync existing state
        ...
        let docs1 = client1_back
            .get_all_documents()
            .await
            .expect("Failed to get documents from client1");
        let docs2 = client2_back
            .get_all_documents()
            .await
            .expect("Failed to get documents from client2");
        let docs3 = client3_back
            .get_all_documents()
            .await
            .expect("Failed to get documents from client3");
        ...
        assert_eq!(
            final_content1, final_content2,
            "Client1 and Client2 should have same final content"
        );
        assert_eq!(
            final_content2, final_content3,
            "Client2 and Client3 should have same final content"
        );

        // Verify the document has been properly updated (not stuck at initial state)
        assert_ne!(
            final_content1["content"], "initial content",
            "Document should not be stuck at initial content"
        );
        ...
        assert!(
            ...
            "Final editor should be one of the clients: {}",
            final_editor
        );

        println!(
            "✅ Simultaneous outage test passed - final state: {:?}",
            final_content1
        );
...
crate::integration_test!(
    test_array_duplication_bug,
    |ctx: TestContext| async move {
        let email = "grace@test.local";

        // Generate proper HMAC credentials
        let (api_key, api_secret) = ctx
            .generate_test_credentials("test-grace")
            .await
            .expect("Failed to generate credentials");
        ...
        client1
            .update_document(doc.id, updated_content)
            .await
            .expect("Failed to update document");
        ...
        let docs1_updated = client1
            .get_all_documents()
            .await
            .expect("Failed to get client1 documents");
        let docs2_updated = client2
            .get_all_documents()
            .await
            .expect("Failed to get client2 documents");
        ...
        assert_eq!(
            tags1.len(),
            2,
            "Client1 should have exactly 2 tags, got: {:?}",
            tags1
        );
        assert_eq!(
            tags2.len(),
            2,
            "Client2 should have exactly 2 tags, got: {:?}",
            tags2
        );
        ...
        assert_ne!(
            tags1[0], tags1[1],
            "Should not have duplicate tags in client1"
        );
        assert_ne!(
            tags2[0], tags2[1],
            "Should not have duplicate tags in client2"
        );
